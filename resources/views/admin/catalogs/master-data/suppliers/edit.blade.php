...
<head>
    ...
    <title>Editar Proveedor | Sistema Web DJ</title>
...
<body class="h-100 d-flex flex-column">
    ...
    <div class="form__item">
        <label for="name" class="form__label form__label--required">Nombre del Proveedor/a</label>
        <div class="input-group ">
            ...
            <input type="text" name="name" id="name" class="form-control @error ('name') is-invalid @enderror"
                placeholder="Ej: Don Pescadon" aria-label="Nombre del Proveedor" aria-describedby="basic-addon1" autofocus value="{{ $supplier->name }}">
        </div>
        @error('name')
    ...
    <div class="form__item">
        ...
        <div class="input-group ">
            ...
            <input type="text" name="rif" id="rif" class="form-control @error ('rif') is-invalid @enderror"
                placeholder="Ej: J-12345678-9" aria-label="RIF del Proveedor" aria-describedby="basic-addon1" autofocus value="{{ $supplier->rif }}">
        </div>
        @error('rif')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
    ...
    <div class="form__item">
        ...
        <div class="input-group ">
            ...
            <textarea name="address" id="address" rows="2" class="form-control @error ('address') is-invalid @enderror"
                placeholder="Ej: Sierra Maestra, Calle 10, entre av. 10 y 11, #10-61 Diagonal al Vivero Girasol, Maracaibo, Venezuela" aria-label="Dirección del Proveedor" aria-describedby="basic-addon1">{{$supplier->address}}</textarea>
        </div>
        @error('address')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
